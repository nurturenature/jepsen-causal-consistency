...
(defn ryw-graph
  ...
  (let [history (->> history
        ...
        adds-by-process-fold (f/make-fold {:name :adds-by-process
                                           ...
                                           :combiner (fn [acc chunk]
                                                       (merge-with set/union
                                                                   acc chunk))})
        add-by-value-fold (f/make-fold {:name :add-by-value
                                        :reducer-identity (constantly {})
                                        :reducer (fn [acc op]
                                                   (let [value (.value op)]
                                                     (assoc acc value op)))
                                        :combiner-identity (constantly {})
                                        :combiner merge})
        fused_folds (f/fuse adds-by-process-fold add-by-value-fold)
        [adds-by-process
         add-by-value]   (-> adds
                             (h/fold (:fused fused_folds)))
        graph (->> reads
                   (reduce (fn [g op]
                             (let [process (.process op)
                                   value   (.value op)
